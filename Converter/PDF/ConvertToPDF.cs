...
namespace Test
{
    internal class ConvertToPDF
    {
        ...
        public bool SaveToPDF(ISldWorks swApp, string pathDoc, string savePath, string pdfFileName)
        {
            ...
            if (swModel != null)
            {
                boolstatus = ConvertFile(savePath, pdfFileName);
                swApp.CloseDoc(swModel.GetTitle());
            }
            return boolstatus;
        }

        public bool SaveToPDF(ModelDoc2 swModel, string savePath, string pdfFileName)
        {
            if (swModel != null)
            {
                boolstatus = ConvertFile(savePath, pdfFileName);
        ...
        private bool ConvertFile(string savePath, string pdfFileName)
        {
